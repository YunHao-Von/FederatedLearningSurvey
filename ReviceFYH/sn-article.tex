...
suspicious clients. Cao et al.~\cite{cao2023fedrecover}
proposed the FedRecover method, which enables the acquisition of a new,
clean model after identifying malicious clients without the need to retrain the models of all participating clients. This approach effectively addresses the issues of excessive computational resource consumption and time associated with model reconstruction in federated learning.
...
The most common idea is to filter out adversarial samples from test data using a trained binary classifier~\cite{wang2024defense, wang2024detecting,zhang2023detecting,feinman2017detecting,pang2022two}.  
Feinman et al.~\cite{feinman2017detecting} train a logistic regression classifier using Kernel Density (KD) of features in the last hidden layer, as well as Bayesian Uncertainty (BU) as a basis.
Pang et al.~\cite{pang2022two} suggest coupling confidence with a proposed R-Con as metrics to separate adversarial samples from normal ones.  
Zhang et al.~\cite{zhang2023detecting} utilize a diffusion model to compute the expected perturbation scores(EPS) of samples, enabling the differentiation
between adversarial and normal samples.
Wang el al.~\cite{wang2024detecting} present a novel defense strategy against adversarial attacks by exploring the proximity relationship between
adversarial noise distributions and developing a robust detector, called Perturbation Forgery.
...
According to the learning paradigm of federated learning, the server model cannot access the data from the client devices.
Some detection models require substantial computational resources, which is challenging to meet in edge devices used for federated learning.
These reasons significantly limit the application of such defense methods in federated learning.
